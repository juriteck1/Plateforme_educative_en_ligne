...
// SVG réseau de neurones décoratif — rose → magenta → bleu
function NeuralNet({ opacity = 0.45 }: { opacity?: number }) {
  ...
  return (
    <svg viewBox="0 0 800 500" xmlns="http://www.w3.org/2000/svg"
      ...
      {/* Noeuds colorés avec filtre glow */}
      {nodes.map((n, i) => {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900" dir={isRTL ? 'rtl' : 'ltr'}>
      <style>{`
        ...
        .grad-text {
          background: linear-gradient(135deg, #7C3AED 0%, #C026D3 50%, #2563EB 100%);
        ...
        .check { color: #7C3AED; font-size: 13px; font-weight: 700; }
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-purple-50">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/connexion" className="btn-secondary text-sm px-4 py-2">
              {t.nav.seConnecter}
            </Link>
      ...
      <section className="relative overflow-hidden" style={{ background: 'linear-gradient(160deg, #FAF5FF 0%, #FDF4FF 40%, #EFF6FF 100%)' }}>
        {/* Réseau de neurones en fond */}
        <div className="neural-anim" style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
          <NeuralNet opacity={0.52} />
        </div>
        ...
        <div className="relative max-w-4xl mx-auto px-6 pt-24 pb-20 text-center">
          <div className="badge fade-up mb-6">
            <span>✦</span> {land.badge}
          </div>

          <h1 className="fade-up-1" style={{
            fontSize: 64, fontWeight: 900, lineHeight: 1.05,
            letterSpacing: '-2px', color: '#1E1B4B', marginBottom: 24
          }}>
            {land.titre1}{' '}
            <span className="grad-text">{land.titre2}</span>
          </h1>
          ...
          <div className="fade-up-3 flex flex-col sm:flex-row gap-3 justify-center items-center mb-14">
            <Link href="/inscription?role=enseignant" className="btn-primary text-base px-8 py-3.5">
              {land.cta1} →
            </Link>
            <Link href="/inscription" className="btn-secondary text-base px-8 py-3.5">
              {land.cta2}
            </Link>
          </div>
          ...
          <div className="flex items-center justify-center gap-6 text-sm" style={{ color: '#9CA3AF' }}>
            <span className="flex items-center gap-1.5">
              <span style={{ color: '#10B981', fontSize: 8 }}>●</span> 500+ utilisateurs actifs
            </span>
        ...
        <div className="relative max-w-4xl mx-auto px-6 pb-0">
          <div style={{
            ...
            border: '1px solid #EDE9FE',
            boxShadow: '0 24px 80px rgba(124,58,237,.15), 0 4px 20px rgba(0,0,0,.05)',
            overflow: 'hidden'
          }}>
            ...
            <div style={{ padding: 24, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
              ...
              {[
                { val: '24', label: 'Élèves connectés', bg: '#F5F3FF', color: '#7C3AED' },
                { val: '8',  label: 'Exercices envoyés', bg: '#FDF4FF', color: '#C026D3' },
                { val: '92%',label: 'Taux de réponse',  bg: '#EFF6FF', color: '#2563EB' },
      ...
      <section id="stats" style={{ borderTop: '1px solid #F5F3FF', borderBottom: '1px solid #F5F3FF', padding: '48px 24px', background: 'white' }}>
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            { val: '50+',  label: 'Enseignants' },
            { val: '98%',  label: 'Satisfaction' },
            { val: '4–16', label: 'Ans couverts' },
          ].map(({ val, label }) => (
            <div key={label}>
        ...
        <div className="max-w-6xl mx-auto">
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 40, fontWeight: 900, color: '#1E1B4B', letterSpacing: -1, marginTop: 12, marginBottom: 12, lineHeight: 1.15 }}>
              {land.features.titre}<br />
              <span className="grad-text-2">{land.features.sousTitre}</span>
            </h2>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
            {[
              { emoji: '⚡', bg: '#F5F3FF', featured: false },
              { emoji: '🧠', bg: '#FDF4FF', featured: true  },
              { emoji: '📊', bg: '#EFF6FF', featured: false },
              { emoji: '📋', bg: '#FFF7ED', featured: false },
              { emoji: '🔔', bg: '#F0FDF4', featured: false },
              { emoji: '🌍', bg: '#EFF6FF', featured: false },
            ].map((f, i) => {
              ...
              return (
                <div key={i} className="card" style={{
                  padding: 24,
                  ...(f.featured ? { borderColor: '#DDD6FE', boxShadow: '0 0 0 1px #DDD6FE' } : {})
                }}>
                  {f.featured && (
                    <div style={{ marginBottom: 12 }}>
                      <span style={{ background: '#F5F3FF', color: '#7C3AED', fontSize: 11, fontWeight: 700, padding: '2px 10px', borderRadius: 999 }}>⭐ {land.features.items ? 'Le plus utilisé' : ''}</span>
                    </div>
                  )}
      ...
      <section id="roles" style={{ padding: '96px 24px', background: 'white' }}>
        <div className="max-w-6xl mx-auto">
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 40, fontWeight: 900, color: '#1E1B4B', letterSpacing: -1, marginTop: 12 }}>
              {land.pourQui.titre}
            </h2>
          </div>
              ...
              return (
                <div key={i} className="card" style={{ overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: 24 }}>
                    ...
                    <Link href="/inscription" className="btn-primary" style={{ display: 'block', padding: '10px 0', fontSize: 14 }}>
                      Commencer →
                    </Link>
                  </div>
                </div>
      ...
      <section style={{ padding: '96px 24px', background: 'white' }}>
        <div className="max-w-3xl mx-auto text-center">
          <div style={{
            ...
            border: '1px solid #EDE9FE', borderRadius: 24, padding: '64px 48px',
            position: 'relative', overflow: 'hidden'
          }}>
            {/* Mini neural net déco */}
            <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
              <NeuralNet opacity={0.18} />
            </div>
            <div style={{ position: 'relative' }}>
              <div className="badge mb-6">{land.cta.titre ? '✦ Prêt ?' : '✦ Prêt ?'}</div>
              <h2 style={{ fontSize: 40, fontWeight: 900, color: '#1E1B4B', letterSpacing: -1, marginTop: 12, marginBottom: 12, lineHeight: 1.15 }}>
                {land.cta.titre}
              </h2>
              ...
              <Link href="/inscription" className="btn-primary" style={{ fontSize: 16, padding: '14px 40px' }}>
                {land.cta.btn} →
              </Link>
